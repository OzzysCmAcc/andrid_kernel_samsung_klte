...
void vibe_set_intensity(int intensity)
{
	if (intensity == 0)
		...
		intensity = ((MAX_INTENSITY - intensity) / 79);	// 79 := 10000 / 127
...
static int ss_vibrator_probe(struct platform_device *pdev)
{
	...
	sysfs_create_file(&vib->timed_dev.dev->kobj, &dev_attr_pwm_default.attr);
	sysfs_create_file(&vib->timed_dev.dev->kobj, &dev_attr_pwm_min.attr);
	sysfs_create_file(&vib->timed_dev.dev->kobj, &dev_attr_pwm_max.attr);
	sysfs_create_file(&vib->timed_dev.dev->kobj, &dev_attr_pwm_threshold.attr);
